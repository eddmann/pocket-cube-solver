...
impl Cube {
    ...
    pub fn next_states(self) -> [(Move, Self); 6] {
        return [
...
mod tests {
    ...
    #[test]
    fn move_forward() {
        assert_eq!("WWOOOYYOGGGGWRRWRRYYBBBB", Cube::solved().apply_move(&Move::Forward).to_string());
    }

    #[test]
    fn move_forward_prime() {
        assert_eq!("WWRROWWOGGGGYRRYOOYYBBBB", Cube::solved().apply_move(&Move::ForwardPrime).to_string());
    }
